...
function landingBodyHtml() {
  ...
  return `
<main>
  <p style="font-size:0.8rem;font-weight:700;letter-spacing:0.2em;text-transform:uppercase;color:#22d3ee;margin:0 0 0.75rem">Hardware-aware observability for self-hosted AI fleets</p>
...
// 1. Landing page: add SoftwareApplication JSON-LD in place.
{
  const jsonLd = {
    ...
    description: 'Hardware-aware observability for self-hosted AI fleets. See what your self-hosted AI actually costs: watts and tokens measured together on every node — real cost per model, phantom-load detection and thermal early warning for Ollama, vLLM and llama.cpp.',
    url: ORIGIN,
