...
- [x] **Windows Thermal (v0.7.8):** WMI `MSAcpi_ThermalZoneTemperature` query via wmic. Tenths-of-Kelvin → Celsius. `thermal_source: "wmi"` — annotated as "estimated" in UI.
...
- [x] **Intel CPU thermal (v0.7.8)** — `coretemp` hwmon detection with per-core temp scanning + clock ratio. Generic cpufreq fallback for CPUs without k10temp or coretemp. `thermal_source: "coretemp"` or `"clock_ratio"`.
...
- [x] **Per-model WES normalization (v0.7.8)** — `query_model_baseline()` in store.rs queries 7-day DuckDB median tok/s + watts at Normal thermal. Background task caches baseline on model change. Three-way sync: `model_baseline_tps/wes/samples` on MetricsPayload. Minimum 100 samples before baseline is considered reliable.
