...
1. **Cloud calculation fixes**: `wes_for_payload` ignores `apple_soc_power_w` (live WES/alerts disagree with persisted history on Apple Silicon — fix: `nvidia.or(apple_soc).or(cpu)`); rollup loses the 5-min bucket straddling the 24h cutoff (`ON CONFLICT DO NOTHING` + unfloored cutoff); 7d+ history charts have a permanent 24h hole at the right edge (rollup only populates past 24h — UNION the raw tail or roll up continuously with DO UPDATE); best-variant selection applies the quant quality factor twice to the incumbent (qf² at main.rs ~5100/5250); OTel `export_interval_s` stored but ignored (fixed 30s tick).
