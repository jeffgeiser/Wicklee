...
1. **Agent task supervision (Pass 2, HIGH — partially shipped).** ~24 fire-and-forget `tokio::spawn` background loops swallow panics: if one dies, the agent keeps running while that subsystem goes silent. **Shipped:** `agent/src/supervisor.rs` — a `supervise(name, factory)` helper that runs the task as a child, logs panics/clean-returns with the task name, and restarts with exponential backoff (1s→30s, reset after a 60s healthy run); unit-tested for restart-on-panic and restart-on-return. **Applied to the metrics broadcast loop** (the most critical — its death stops ALL telemetry), a pure infinite loop wrapped with a compiler-checked clone-per-restart prelude (body unchanged). **Remaining:** the Ollama/vLLM/llama.cpp harvester main loops, and `cloud_push`. Two wrinkles to handle for those: (a) the harvester loops contain nested inner `break`s — confirm the outer loop can't fall through before wrapping; (b) `cloud_push` has an INTENTIONAL terminal `break` on 410-Gone (node removed), so it needs a "don't restart" signal — extend the supervisor to take a future returning `ControlFlow`/`bool` rather than blindly restarting on clean return. Acceptance: panic a harvester in a test build and confirm it logs + recovers; an intentional cloud_push exit must NOT restart-spin.
