...
1. **Agent task supervision (Pass 2, HIGH).** ~24 fire-and-forget `tokio::spawn` background loops — the broadcast loop, the Ollama/vLLM/llama.cpp harvesters, and the cloud-push loop — swallow panics: if one dies, the agent keeps running while that subsystem goes silent (stale or no metrics, no error surfaced). Fix: a restart supervisor. Refactor each critical loop into a re-runnable closure factory (`Fn() -> impl Future`) and spawn it under a helper that awaits the inner `JoinHandle`, logs panics/returns with the task name, and restarts after a backoff (e.g. 5s). Non-critical one-shot spawns can use a lighter log-on-panic wrapper. Acceptance: kill/panic a harvester in a test build and confirm it logs + recovers; the broadcast loop must never die silently.
