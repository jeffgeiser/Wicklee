...
The SPA served every route from one index.html shell with identical landing-page metadata — duplicate titles/snippets in Google, wrong social unfurls everywhere (Slack/X/LinkedIn don't run JS), an empty `<div>` for the AI crawlers robots.txt explicitly welcomes, and a robots.txt that advertised a sitemap.xml which didn't exist (the SPA fallback served HTML for it). Phase 1: per-route title/description/canonical/og:url swapping on navigation (`src/utils/pageMeta.ts`, with BlogPost resolving from frontmatter), missing `og:type`/`og:site_name`/`twitter:card`/canonical added to the shell, and a sitemap generator vite plugin (static routes + blog posts with lastmod). Phase 2: build-time static HTML for the content routes via `scripts/generate-static-pages.mjs` (postbuild) — `dist/blog/{slug}/index.html`, blog listing, and `/docs`, each the built SPA shell with swapped metadata, the article HTML pre-injected into `#root`, and JSON-LD (`BlogPosting`/`TechArticle`/`SoftwareApplication`). nginx needed zero changes (`try_files $uri $uri/` prefers the emitted directory indexes); React takes over on load. `/metrics` deliberately excluded — nginx exact-matches it to the cloud's Prometheus scrape proxy. Deliberately NOT a framework migration: the dashboard has no SEO value; revisit Astro only if the marketing surface grows. Follow-up: a real 1200×630 og:image asset (twitter:card is `summary` until then).
...
1. **Agent task supervision (Pass 2, HIGH — partially shipped).** ~24 fire-and-forget `tokio::spawn` background loops swallow panics: if one dies, the agent keeps running while that subsystem goes silent. **Shipped:** `agent/src/supervisor.rs` — a `supervise(name, factory)` helper that runs the task as a child, logs panics/clean-returns with the task name, and restarts with exponential backoff (1s→30s, reset after a 60s healthy run); unit-tested for restart-on-panic and restart-on-return. **Applied to the metrics broadcast loop** (the most critical — its death stops ALL telemetry), a pure infinite loop wrapped with a compiler-checked clone-per-restart prelude (body unchanged). **Shipped (2):** `supervise_until(name, factory)` — the future returns `ControlFlow`, where `Break(())` is a deliberate permanent stop (no restart) and `Continue(())`/panic restarts with backoff; unit-tested. `cloud_push` now runs under it, with its two terminal exits (broadcast channel closed; 410-Gone after node removal) returning `Break` so they don't restart-spin. **Remaining: the Ollama/vLLM harvester main loops** (the probe sub-tasks are non-critical baseline measurement — lower priority). These must use `supervise_until`, NOT plain `supervise`: each contains `if port_rx.changed().await.is_err() { return; }` — a TERMINAL exit when the discovery watch-channel's senders drop (shutdown), which plain restart would spin on. Conversion: wrap with a compiler-checked clone-per-restart prelude (same as the broadcast loop), and map every `changed().await.is_err()` return and any other intentional exit to `return ControlFlow::Break(())`; genuine panics then restart while shutdown stops cleanly. Acceptance: panic a harvester in a test build and confirm it logs + recovers; a watch-channel-closed shutdown must NOT restart-spin.
...
4. **Pass 3 — frontend state correctness (not yet started).** The third review pass: SSE/stream reconnection and token-refresh races, stale-closure bugs in the React hooks, rolling-buffer/smoothing-store correctness across tab switches and node churn, error-boundary coverage so one bad payload can't blank the dashboard, and cleanup of effects/listeners on unmount. Run the same survey → severity-ranked findings → tested fixes loop used for Passes 1 & 2.
